...
def list_unique_json_files(s3_client, bucket, prefix, show_progress=False, total_expected=None):
    ...
    try:
        ...
        # If the bucket doesn't exist, treat as no files
        pass
    except Exception as e:
        # If the prefix/folder doesn't exist, treat as no files
    ...
    if pbar:
        pbar.close()
    # If no files found at all, return 0
    return len(unique_names) if found_any else 0
...
def main(docket_id):
    ...
    try:
        ...
        if e.response is not None and e.response.status_code == 404:
            click.echo(f"Error: Docket ID '{docket_id}' not found.", err=True)
            exit(1)
